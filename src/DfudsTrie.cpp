...
uint32_t DfudsTrieLCP::findLCP(const char *key, uint8_t *lcp) const {
  uint32_t offset = 1;
  if (lcp && *lcp) {
...
uint32_t DfudsTrieLCP::rightNearFindLCP(const char *key, uint8_t *lcp) const {
  uint32_t offset = 1;
  if (lcp && *lcp) {
    offset = _offsets[*lcp - 1];
    key += *lcp;
  ...
  uint32_t rank = _keyRank(offset - 1) + 1;
  return rank;
}

uint32_t DfudsTrie::findLCP(const char *key, uint8_t *lcp) const {
  return find(key);
}

uint32_t DfudsTrie::rightNearFindLCP(const char *key, uint8_t *lcp) const {
  return rightNearFind(key);
}

uint32_t DfudsTrie::find(const char *key) const {
  uint32_t offset = 1;
...
uint32_t DfudsTrie::rightNearFind(const char *key) const {
  uint32_t offset = 1;
  int keyLen = 0;
...
void DfudsTrie::select(uint32_t rank, string &key) const {
  assert(0 < rank && rank <= _is_keys.rank1(_is_keys.count() - 1));
  key.clear();
  // + 1 to turn offset to rank, -1 to move to the node before.
  uint32_t nodeOffset = _is_keys.select1Naive(rank) + 1 - 1;
  // + 1 to move to the beginning of next node.
  uint32_t id = _dfuds.select1Naive(nodeOffset) + 1;

  while (id != 1) {
    uint32_t parent = _parent(id);
...
bool DfudsTrie::_followKey(const char *key, 
    uint32_t &id, int *prefixLen, int *depth) const {
...
uint8_t DfudsTrie::_childLowerBound(uint32_t id, uint8_t ch) const {
  if (_dfuds[id])  return 0;

  uint32_t rankOpen = _dfuds.rank0(id) - 1;
  uint32_t base = rankOpen - 1;
...
uint8_t DfudsTrie::_childRank(uint32_t id, uint8_t ch) const {
  if (_dfuds[id])  return 0;
...
uint32_t DfudsTrie::_childSelect(uint32_t id, uint8_t rank) const {
  uint32_t offsetClose = id + _degree(id);
  uint32_t tmpOffset = offsetClose - rank;
  uint32_t nextOffset = _dfuds.findClose(tmpOffset) + 1;
  return nextOffset;
}

uint32_t DfudsTrie::_generalizedSibling(uint32_t id) const {
  uint32_t offset = id;
  while (true) {
...
uint8_t DfudsTrie::_degree(uint32_t id) const {
  int offset = id;
...
uint8_t DfudsTrie::_label(uint32_t parent, uint32_t child) const {
  uint32_t openOffset = _dfuds.findOpenNaive(child - 1);
  int degree = _degree(parent);
  uint8_t childRank = parent + degree - openOffset;

  // -1 for the head open, -1 to turn rank to offset.
  uint32_t base = _dfuds.rank0(parent) - 1 - 1;
  return _labels[base + childRank - 1];
}

uint32_t DfudsTrie::_parent(uint32_t id) const {
  uint32_t openOffset = _dfuds.findOpenNaive(id - 1);
  uint32_t closeRank = _dfuds.rank1(openOffset);
  if (closeRank == 0)  return 1;
  uint32_t result = _dfuds.select1Naive(closeRank) + 1;
  return result;
}

uint32_t DfudsTrie::_keyRank(uint32_t id) const {
  uint32_t close_offset = id + _degree(id);
  uint32_t key_offset = _dfuds.rank1(close_offset) - 1;
  uint32_t rank = _is_keys.rank1(key_offset);
  return rank;
}
